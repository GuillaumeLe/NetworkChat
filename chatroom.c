...
char* clt_authentication(int clt_sock){
  ...
  for(attemp=0; attemp < MAX_AUTH_ATTEMPS; attemp++){
     ...
     */
     ...
     unsigned char size;
     void *login = malloc(100);
     char msg[] = "Authentication request";
     send_msg(clt_sock, AUTH_REQ,sizeof(msg),msg);
     recv_msg(clt_sock, &code, &size, &login);
     printf("%d\n", code);
     if (code == AUTH_RESP){
         printf("Receive AUTH_RESP\n");
         printf("Authentication of %s\n", (char *)login);
         char msg2[] = "Access Authorized";
         send_msg(clt_sock, ACCESS_OK, sizeof(msg2), msg2);
         return login;
      }else{
        printf("AUTH_RESP not received\n");
        char msg3[] = "Access denied";
        send_msg(clt_sock, ACCESS_DENIED,sizeof(msg3), msg3);
